        ...
        for cnt,pair in enumerate(zip(Elem,S)):
            if cnt == len(S)-1:
                ...
                if cnt == 0: # first node on left side
                    s4[ide] += 1/2*( S[cnt+1]*(Elem[cnt+1]-Elem[cnt]) + 0 ) # O(h)
                  # s4[ide] += 1/2*( S[cnt+1]*(Elem[cnt+1]-Elem[cnt]) + S[cnt]*(Elem[cnt+1]-Elem[cnt]) ) # Forward Euler on second derivative,  O(h)
                else:
