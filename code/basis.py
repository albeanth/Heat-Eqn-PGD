...
def updateR(R,X,dx,NumEnr):
    r1=0.0; r2=0.0; r3=0.0;
    if NumEnr == 1:
        r4 = [0.0]; r5 = [0.0]
    else:
        r4=np.zeros(len(X)); r5=np.zeros(len(X))
    ...
    if NumEnr > 1: # past first iteration,
...
def updateS(S,T,dt,NumEnr):
    s1=0.0; s2=0.0; s3=0.0;
    if NumEnr == 1:
        s4 = [0.0]; s5 = [0.0]
    else:
        s4=np.zeros(len(T)); s5=np.zeros(len(T))  # note, that the first elem of s4 and s5 will ALWAYS be zero. attributed to initial guess being zeros.
...
def updateW(W,K,dk,k,NumEnr):
    w1=0.0; w2=0.0; w3=0.0;
    if NumEnr == 1:
        w4 = [0.0]; w5 = [0.0]
    else:
        w4=np.zeros(len(K)); w5=np.zeros(len(K))
